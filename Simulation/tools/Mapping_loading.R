Mapping_Loading<-function(L,LL,Path){
  Mer_mat<-cbind(LL,L)
  cor_mat<-cor(Mer_mat)
  p<-pheatmap::pheatmap(cor_mat,cluster_rows = F,cluster_cols = F)
  cowplot::save_plot(paste0(Path,"/cor_loading.png"), p,
  ...
  cowplot::save_plot(paste0(Path,"/cor_diagonal_loading.png"), p,
  ...
  mapped_matrix<-L[,L_var_index]
  
  Mer_mat<-cbind(LL,mapped_matrix)
  cor_mat<-cor(Mer_mat)
  A<-cor_mat[1:ncol(LL),(ncol(LL)+1):ncol(Mer_mat)]
  mapped_matrix<-sweep(mapped_matrix, 2,sign(diag(A)), FUN ="*")
  
  
  return(list('mapped_matrix'=mapped_matrix,"Latent_Matrix" = L))
}
